...
const slides = document.querySelector(".slider__items");
const nextBtn = document.querySelector(".slider__pagination_next");
const prevBtn = document.querySelector(".slider__pagination_prev");
let width = parseInt(window.getComputedStyle(slides).width) - parseInt(window.getComputedStyle(sliderWrapper).width); //1358
let offset = 0;
let step = document.body.clientWidth > 768 ? 3 : 6;
let count = 0;
...
function resetSlider() {
    step = document.body.clientWidth > 768 ? 3 : 6;
    count = 0;
    slides.style.transform = `translateX(0px)`;
    width = parseInt(window.getComputedStyle(slides).width) - parseInt(window.getComputedStyle(sliderWrapper).width);
    offset = 0;
...
new ResizeObserver(resetSlider).observe(sliderWrapper);
...
const endtime = new Date(year, 0, 1);
...
setTime(endtime);
setInterval(() => setTime(endtime), 1000);
...
// const gifts = [
//     {data: "work", img: "./images/gift-for-work.png", header: "For work", description: "Console.log Guru"}
// ]
...
const randomArr = [...Array(4)];
let i = 0;
fillRundomArr();


function rundomNum() {
    return Math.round(Math.random() * (gifts.length - 1))
}



function fillRundomArr() {
    for (i; i < randomArr.length; i++) {
        let num = rundomNum();
        if (randomArr.some(item => item === num)) {
            fillRundomArr();
            break;
        } else {
            randomArr[i] = num;
...
randomArr.forEach(card => {
    ...
    cardItems.innerHTML += `
    <div class="best-gifts__item" data-app=${gifts[card].category.replace("For ", "").toLowerCase()}>
    <div class="best-gifts__item-img">
        <img src="./images/gift-${gifts[card].category.replace("For ", "for-").toLowerCase()}.png" alt="gift-${gifts[card].category.replace("For ", "for-").toLowerCase()}">
    </div>
    <div class="best-gifts__item-descr">
        <div class="best-gifts__item-descr-header ${gifts[card].category.replace("For ", "").toLowerCase()}">${gifts[card].category}</div>
        <div class="best-gifts__item-descr-text">${gifts[card].name}</div>
    </div>
    </div>
    `
})
